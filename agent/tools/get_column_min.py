...
def get_column_min_to_wrap(target_table: str, field: str):
    A: Table = get_table(target_table)
    column: list[str] = A.get_column(field)
    try:
        column_f: list[float] = [float(x) for x in column]
    except Exception:
        return "Error: you can calculate min only for numerical values"

    return str(StatFuncs.min(column_f))

get_column_min = path_validator(get_column_min_to_wrap)

schema_get_column_min: ChatCompletionToolUnionParam = {
    "type": "function",
    "function": {
        "name": "get_column_min",
        "description": "Calculate the min of the given column. The values MUST be numerical",
        "parameters": {
            "type": "object",
            "properties": {
                "target_table": {
                ...
                "field": {
                    "type": "string",
                    "description": "The corresponding column is the one whose min will be computed"
                },
            },
