...
This is an AI-Agent built with [OpenAI SDK](https://developers.openai.com/api/docs/libraries) and [OpenRouter](https://openrouter.ai/). It can perform basic statistical operations on the datasets saved locally in the form of `.csv` files. The agent API is written in Python3, as well as the tools the agent can use. Those tools rely on a shared library implemented in C and exposed to Python via [ctypes](https://docs.python.org/3/library/ctypes.html).

## How to install and run the project - Linux, Mac, Windows

1. Clone the repository
```bash
git clone https://github.com/Werian97/Statistical-AI-Agent.git
cd Statistical-AI-Agent
```
or download it as a ZIP and extract it. All commands below should be run from the project root.
...
4. Install python requirements. You can do this in 2 ways:
    ...
    uv sync
    ```
5. If you are running all of this from Mac or Windows there is still an issue to fix: the Python wrapper currently contains the path to the shared library, so it must be adjusted according to the operating system.
In order to do that: open the file `python_API/C_lib_API.py` and edit line 5. Replace it with the following
    - Mac:
    ```python
...
6. Run the command
```bash
python3 main.py "Perform an analysis of the dataset house_prices.csv"
```
Or, if you used [uv](https://docs.astral.sh/uv/), use
```bash
uv run main.py "Perform an analysis of the dataset house_prices.csv"
```
...
```
and replace \<prompt> with your actual prompt between quotes. Sky is the limit.
...
The agent can access any CSV file in `data/small_data` and `data/big_data`. By default there is only `data/small_data/house_prices.csv`, but you can add as many tables as you want. Make sure to name them with the final `.csv` extension.
...
This project was a ton of fun. I used everything I learned in the last 3 months and I built something that no one taught me. The creative process started with this thought: I want to write something in C and make Python use it. That was my goal.
